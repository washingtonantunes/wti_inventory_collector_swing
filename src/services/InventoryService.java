...
public class InventoryService {
	...
	public static InventoryTest createChangeInventoryDelivery(InventoryTest inventory_) {
		...
		if (inventory_.isWorkPositionBoolean()) {
			inventory_.setChangesEquipment(
					"Equipamento alocado na posição: " + inventory_.getWorkPosition().getWorkPoint());
		...
		// WorkPosition
		if (inventory_.isWorkPositionBoolean()) {
			inventory_.setChangesWorkPosition(
					"Posição ocupada pelo equipamento: " + inventory_.getEquipment().getSerialNumber());
		}
		return inventory_;
	}
}
